...
fullAnova_id <- anovaBF(mean ~ modality + condition + modality:condition + id,
             data=data_grp,
             whichRandom = "id")

fullAnova_all <- anovaBF(errorTime ~ modality + condition + modality:condition + id,
             data=data,
             whichRandom = "id")

# T-tests -----------------------------------------------------------------
W_data_foot <- filter(all_data, condition == "W" & modality == "foot") %>%
    select(id, errorTime) %>% 
